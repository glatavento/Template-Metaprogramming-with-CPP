...
\begin{itemize}
  \item 别名模板std::enable\_if\_t，这是访问std::enable\_if<B, T>::type成员类型的方法。其定义如下：

\begin{cpp}
template <bool B, typename T = void>
using enable_if_t = typename enable_if<B,T>::type;
\end{cpp}
  \item 两个变量模板std::is\_integral\_v和std::is\_floating\_point\_v是访问数据成员的方法，std::is\_integral<T>::value和std::is\_floating\_point<T>::value。std::is\_integral和std::is\_float\_point类是标准类型特征，分别检查类型是整型还是浮点型。
\end{itemize}
